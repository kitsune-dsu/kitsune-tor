...
{
  static struct event *launch_event; /**DSU other */
  struct timeval timeout;
  if (!get_options()->ServerDNSDetectHijacking)
    return;
  if (!launch_event)            /**DSU other 2  */
    launch_event = tor_malloc_zero(sizeof(struct event));
  ...
  evtimer_set(launch_event, launch_test_addresses, NULL); /**DSU other  */
  timeout.tv_sec = 30;
  timeout.tv_usec = 0;
  if (evtimer_add(launch_event, &timeout)<0) { /**DSU other  */
