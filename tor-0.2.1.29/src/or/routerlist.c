...
router_get_routerlist(void)
{
  if (PREDICT_UNLIKELY(!routerlist)) {
    ...
    /* All strdups in this section are DSU changes, so that the local pointers
       don't expire when we update. */
    /**DSU other 5  */
    routerlist->desc_store.fname_base = strdup("cached-descriptors");
    routerlist->desc_store.fname_alt_base = strdup("cached-routers");
    routerlist->extrainfo_store.fname_base = strdup("cached-extrainfo");

    routerlist->desc_store.type = ROUTER_STORE;
    routerlist->extrainfo_store.type = EXTRAINFO_STORE;

    routerlist->desc_store.description = strdup("router descriptors");
    routerlist->extrainfo_store.description = strdup("extra-info documents");
  }
  return routerlist;
}
