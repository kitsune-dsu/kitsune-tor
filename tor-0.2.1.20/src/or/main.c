...
do_main_loop(void)
{
  ...
  if (kitsune_is_updating())     /**DSU control  */
    log_debug(LD_DSU, "Re-setting signals");
  handle_signals(1);
  ...
  for (;;) {
    ...
    /* DSU change */
    kitsune_update("mainloop"); /**DSU updatepoint */
    /* end DSU change */
...
control_signal_act(int the_signal)
{
  switch (the_signal)
    ...
    case SIGCLEARDNSCACHE:
      ...
      break;
    case SIGUPDATE:             /**DSU other 2 */
      signal_callback(0,0,(void*)(uintptr_t)SIGUPDATE);
      break;
    default:
...
signal_callback(int fd, short events, void *arg)
{
  ...
  switch (sig)
    ...
    case SIGUSR1:
      ...
      break;
    case SIGUPDATE: /**DSU other 7 */
      if(!get_options()->DoDSU)
        log_warn(LD_DSU, "Controller asked us to update, but we don't "
                 "support that. Ignoring.");
      else
        signal_update();
      break;
    case SIGUSR2:
      if (get_options()->DoDSU) { /* DSU other 3 */
        signal_update();	  
      } else {
      switch_logs_debug();
...
handle_signals(int is_parent)
{
  ...
  static int signals[] = {
    ...
    SIGUSR1, /* dump stats */
    SIGUSR2, /* go to loglevel debug, or initiate DSU */
    ...
    -1 };
  static struct event *signal_events = NULL; /**DSU other 3 */

  if (!signal_events)
    signal_events = malloc(sizeof(struct event) * 16); /* bigger than it has to
                                                          be. */
...
tor_init(int argc, char *argv[])
{
  ...
  if (kitsune_is_updating()) {   /**DSU control 2 */
    log_debug(LD_DSU, "Entering tor_init after dynamic update");
    kitsune_do_automigrate(); /**DSU data 2 */
    config_migrate();
    networkstatus_migrate();    /**DSU data_add */
  }
 ...
 /* give it somewhere to log to initially */
  ...
  log(LOG_NOTICE, LD_GENERAL, "Tor v%s%s. This is experimental software. "
      ...
      (kitsune_is_updating())?"(update)":"", get_uname()); /**DSU control */
