...
#include "orconfig.h"
#include <kitsune.h>             /* DSU change */
#define UINT64_MAX (~0ull)      /**DSU other */
...
#define SIGUPDATE 131           /**DSU other */
...
typedef struct connection_t {
  ...
  struct event * E_OPAQUE read_event; /**< Libevent event structure. */ /**DSU xfgen */
  struct event * E_OPAQUE write_event; /**< Libevent event structure. */ /**DSU xfgen */
...
typedef struct {
   ...
   * routers in routers or old_routers. */
  struct digest_sd_map_t * E_OPAQUE desc_by_eid_map; /**DSU xfgen */
...
#define LOG_PROTOCOL_WARN (get_options()->ProtocolWarnings ? \
  ...
  /** If true, SIGUSR2 initiates a dynamic software update, rather
      than upping the LogLevel to debug. */
  int DoDSU;                    /**DSU other */

  /** The path to the shared object we will dynamically update to.  */
  char *DSUTarget;              /**DSU other */
