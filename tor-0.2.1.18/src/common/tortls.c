...
typedef struct tor_tls_context_t {
  int refcnt;
  SSL_CTX * E_OPAQUE ctx; /**DSU xfgen */
  X509 * E_OPAQUE my_cert; /**DSU xfgen */
  X509 *E_OPAQUE my_id_cert; /**DSU xfgen */
  crypto_pk_env_t *key;
} tor_tls_context_t;
...
struct tor_tls_t {
  ...
  SSL * E_OPAQUE ssl; /**< An OpenSSL SSL object. */ /**DSU xfgen */
 ...
 /** True iff we should call negotiated_callback when we're done reading. */
  ...
  void * E_OPAQUE callback_arg; /**DSU xfgen */
};
...
static const char CLIENT_CIPHER_LIST[] E_MANUAL_MIGRATE = /**DSU data */
#include "./ciphers.inc"
  ;
#undef CIPHER
#undef XCIPHER
...
static const cipher_info_t CLIENT_CIPHER_INFO_LIST[] E_MANUAL_MIGRATE = { /**DSU data */
...
static const int N_CLIENT_CIPHERS E_MANUAL_MIGRATE = /**DSU data */
...
tor_tls_new(int sock, int isServer)
{
  ...
  if (!isServer) {              /* DSU backported tls fix *//**DSU other */
